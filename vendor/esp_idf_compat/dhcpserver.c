...
// Ratspeak compatibility backport: see README.md in vendor/esp_idf_compat.
// Replaces the pinned SDK archive member; it does not add another DHCP server.
...
static u8_t parse_options(u8_t *optptr, s16_t len)
{
    ...
    if (len <= 0) return DHCPS_STATE_IDLE;
    u8_t *end = optptr + len;
    u16_t type = 0;
    ...
    while (optptr < end) {
        ...
        if (*optptr == DHCP_OPTION_END) break;
        // Subtraction avoids forming a pointer past the buffer while checking
        // an untrusted length. Preserve upstream's ignore-truncated-tail policy.
        const size_t remaining = (size_t)(end - optptr);
        if (remaining < 2) break;
        const u8_t opt_len = optptr[1];
        if ((size_t)opt_len > remaining - 2) break;

        switch ((s16_t) *optptr) {

            case DHCP_OPTION_MSG_TYPE:	//53
                if (opt_len >= 1) type = optptr[2];
                break;
