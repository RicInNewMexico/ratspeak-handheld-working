...
#if MBEDTLS_VERSION_NUMBER != 0x021C0700 || defined(MBEDTLS_USE_PSA_CRYPTO) || defined(MBEDTLS_SSL_PROTO_SSL3)
#error "Review/remove the non-PSA Mbed TLS 2.28.7 Finished backport for this SDK"
#endif
    ...
    mbedtls_ssl_context *ssl, unsigned char *buf, int from)
{
    ...
    ret = mbedtls_md5_finish_ret(&md5, padbuf);
    if (ret == 0) ret = mbedtls_sha1_finish_ret(&sha1, padbuf + 16);

    if (ret == 0) ret = ssl->handshake->tls_prf(session->master, 48, sender,
                            padbuf, 36, buf, len);

    if (ret != 0) mbedtls_platform_zeroize(buf, len);
